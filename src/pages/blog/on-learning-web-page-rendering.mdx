...
<Notes>The site belongs to Vercel from their demo page. https://next-blog-wordpress.vercel.app/</Notes>
...
<Notes>By looking at the cons, Next js actually try to eliminate those problems by introducing Incremental Static Generation (ISG). Which fundamentally is SSG, but with an extra capability to implement SSG on a per-page basis. This means you don't need to rebuild the entire site if there are changes happen.</Notes>
